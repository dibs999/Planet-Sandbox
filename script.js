...
const dayIndicator = document.getElementById("daytime-indicator");
const weatherIndicator = document.getElementById("weather-indicator");
const autoWeatherToggle = document.getElementById("auto-weather");
const rainBtn = document.getElementById("rain-btn");
const meteorBtn = document.getElementById("meteor-btn");
const statHumans = document.getElementById("stat-humans");
const statAliens = document.getElementById("stat-aliens");
const statHumanHouses = document.getElementById("stat-human-houses");
const statAlienHouses = document.getElementById("stat-alien-houses");
...
const WEATHER_LABELS = {
  [WEATHER_TYPES.CLEAR]: "klar",
  [WEATHER_TYPES.RAIN]: "Regen",
  [WEATHER_TYPES.METEOR]: "Meteorsturm",
};
...
let isPaused = false;
let stepOnce = false;
let simSpeed = speedControl ? parseFloat(speedControl.value) : 1;
let tickAccumulator = 0;
let timeOfDay = DAY_LENGTH * 0.25;
let isNight = false;
const weatherState = {
  current: WEATHER_TYPES.CLEAR,
  timer: 0,
  nextEventIn: randomRange(900, 1600),
  auto: autoWeatherToggle ? autoWeatherToggle.checked : true,
};
...
function resetWorld() {
  ...
  weatherState.auto = autoWeatherToggle ? autoWeatherToggle.checked : weatherState.auto;
  weatherState.current = WEATHER_TYPES.CLEAR;
  weatherState.timer = 0;
  weatherState.nextEventIn = randomRange(900, 1600);
  tickAccumulator = 0;
  timeOfDay = DAY_LENGTH * 0.25;
  clearEventLog();
  logEvent(`Neustart auf ${activePlanet.name}.`);
  setWeather(WEATHER_TYPES.CLEAR, 0, { silent: true });
  advanceDayNight(0);
  updateStatistics();
}
...
function setupUI() {
  ...
  if (speedControl) {
    simSpeed = parseFloat(speedControl.value);
    updateSpeedDisplay();
  ...
  if (autoWeatherToggle) {
    autoWeatherToggle.addEventListener("change", () => {
      weatherState.auto = autoWeatherToggle.checked;
      logEvent(
        weatherState.auto
          ? "Das Wetter wird wieder automatisch gesteuert."
          : "Automatisches Wetter ist deaktiviert."
      );
    });
  }
  ...
  if (rainBtn) {
    rainBtn.addEventListener("click", () => {
      setWeather(WEATHER_TYPES.RAIN, randomRange(320, 520));
    });
  }

  if (meteorBtn) {
    meteorBtn.addEventListener("click", () => {
      setWeather(WEATHER_TYPES.METEOR, randomRange(180, 260));
    });
  }
}
...
function updateSpeedDisplay() {
  ...
  const formatted = Number.isInteger(simSpeed)
    ? `${simSpeed.toFixed(0)}×`
    : `${simSpeed.toFixed(1)}×`;
...
function buildHumanHouse(x, y) {
  ...
  logEvent("Die Menschen errichten ein neues Haus.");
}
...
function buildAlienDome(x, y) {
  ...
  logEvent("Die Aliens schließen eine neue Biokuppel ab.");
}
...
function setWeather(type, duration = 0, { silent = false } = {}) {
  weatherState.current = type;
  weatherState.timer = duration;
  if (weatherState.auto) {
    weatherState.nextEventIn = randomRange(900, 1800);
  }
  if (weatherIndicator) {
    weatherIndicator.textContent = WEATHER_LABELS[type] || type;
  }
  if (!silent) {
    switch (type) {
      case WEATHER_TYPES.CLEAR:
        logEvent("Der Himmel klart auf.");
        break;
      case WEATHER_TYPES.RAIN:
        logEvent("Dunkle Wolken bringen Regen über den Planeten.");
        break;
      case WEATHER_TYPES.METEOR:
        logEvent("Ein Meteorsturm zieht über den Himmel!");
        break;
      default:
        break;
    }
...
function runWeatherTick(delta = 1) {
  if (weatherState.current === WEATHER_TYPES.CLEAR) {
    if (weatherState.auto) {
      weatherState.nextEventIn -= delta;
      if (weatherState.nextEventIn <= 0) {
        const next = Math.random() < 0.7 ? WEATHER_TYPES.RAIN : WEATHER_TYPES.METEOR;
        const length =
          next === WEATHER_TYPES.RAIN ? randomRange(360, 540) : randomRange(160, 260);
        setWeather(next, length);
      }
    }
    return;
  }
  ...
  if (weatherState.current === WEATHER_TYPES.RAIN) {
    spawnRainDrops(Math.max(1, Math.round(delta * 4)));
  } else if (weatherState.current === WEATHER_TYPES.METEOR) {
    spawnMeteorStrikes(Math.max(1, Math.round(delta * 2)));
  }

  if (weatherState.timer <= 0) {
    setWeather(WEATHER_TYPES.CLEAR);
  }
}

function spawnRainDrops(amount) {
  const rows = Math.min(5, GRID_HEIGHT);
  for (let i = 0; i < amount; i++) {
    const x = Math.floor(Math.random() * GRID_WIDTH);
    ...
    if (!target) continue;
    if (
      target.type === TYPES.EMPTY ||
      target.type === TYPES.SMOKE ||
      target.type === TYPES.STEAM
    ) {
      setCell(x, y, createCell(TYPES.WATER));
    }
  }
...
function spawnMeteorStrikes(amount) {
  const rows = Math.min(6, GRID_HEIGHT);
  for (let i = 0; i < amount; i++) {
    const x = Math.floor(Math.random() * GRID_WIDTH);
    const y = Math.floor(Math.random() * rows);
    const type = Math.random() < 0.5 ? TYPES.FIRE : TYPES.LAVA;
    const payload =
      type === TYPES.FIRE
        ? createCell(TYPES.FIRE, { life: 30 })
        : createCell(TYPES.LAVA, { life: 90 });
    setCell(x, y, payload);
    const shockRadius = 1 + Math.floor(Math.random() * 2);
    for (let dy = -shockRadius; dy <= shockRadius; dy++) {
      for (let dx = -shockRadius; dx <= shockRadius; dx++) {
        const nx = x + dx;
        const ny = y + dy;
        if (!inBounds(nx, ny)) continue;
        if (Math.abs(dx) + Math.abs(dy) > shockRadius) continue;
        const existing = getCell(nx, ny);
        if (existing.type === TYPES.PLANT) {
          setCell(nx, ny, createCell(TYPES.WOOD));
        }
      }
    }
...
function advanceDayNight(ticks) {
  timeOfDay = (timeOfDay + ticks + DAY_LENGTH) % DAY_LENGTH;
  const normalized = timeOfDay / DAY_LENGTH;
  const daylight = (Math.sin(normalized * Math.PI * 2 - Math.PI / 2) + 1) / 2;
  const brightness = 0.35 + daylight * 0.65;
  isNight = brightness < 0.5;
  if (canvas) {
    canvas.style.background = adjustSkyColor(activePlanet.skyColor, brightness);
  }
  if (dayIndicator) {
    let descriptor = "Nacht";
    if (brightness >= 0.7) {
      descriptor = "Tag";
    } else if (brightness >= 0.55) {
      descriptor = "Dämmerung";
    } else if (brightness >= 0.45) {
      descriptor = "Abend";
    }
    dayIndicator.textContent = descriptor;
  }
}

function adjustSkyColor(hex, brightness) {
  const { r, g, b } = hexToRgb(hex);
  const clamped = clamp(brightness, 0, 1);
  const darkness = 1 - clamped;
  let rAdj = Math.round(r * clamped + 12 * darkness);
  let gAdj = Math.round(g * clamped + 16 * darkness);
  let bAdj = Math.round(b * clamped + 28 * darkness);
  const highlight = Math.round(18 * clamped);
  rAdj = clamp(rAdj + highlight, 0, 255);
  gAdj = clamp(gAdj + Math.round(highlight * 0.75), 0, 255);
  bAdj = clamp(bAdj + Math.round(highlight * 0.9), 0, 255);
...
function updateStatistics() {
  ...
  for (let i = 0; i < grid.length; i++) {
    ...
    switch (cell.type) {
      ...
      case TYPES.HOUSE_HUMAN:
        totals.humanHouses += 1;
        break;
      case TYPES.HOUSE_ALIEN:
        totals.alienHouses += 1;
        break;
      case TYPES.WATER:
        totals.water += 1;
        break;
  ...
  const humanStructures = Math.round(totals.humanHouses / 9);
  const alienStructures = Math.round(totals.alienHouses / 10);

  if (statHumans) statHumans.textContent = totals.humans;
  if (statAliens) statAliens.textContent = totals.aliens;
  if (statHumanHouses) statHumanHouses.textContent = humanStructures;
  if (statAlienHouses) statAlienHouses.textContent = alienStructures;
  if (statWater) statWater.textContent = totals.water;
  if (statFire) statFire.textContent = totals.fire;
}
...
function loop(now = performance.now()) {
  const deltaRaw = now - lastFrameTime;
  const deltaMs = Math.min(Math.max(deltaRaw, 0), 250);
  lastFrameTime = now;

  if (!isPaused) {
    tickAccumulator += (deltaMs / 16.6667) * simSpeed;
  }

  if (stepOnce) {
    tickAccumulator = Math.max(tickAccumulator, 1);
  }

  const ticks = Math.min(Math.floor(tickAccumulator), 12);
  if (ticks > 0) {
